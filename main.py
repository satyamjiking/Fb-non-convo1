...
logger = logging.getLogger("FINAL_STEALTH_BOT")
...
def build_session(config: Dict) -> requests.Session:
    ...
    s = requests.Session()
    s.headers.update({
        # Stealth Mode User-Agent: इसे असली मोबाइल ब्राउज़र जैसा दिखने के लिए
    ...
    logger.info("Attempting login check using active cookies (Stealth Mode)...")
    ...
    try:
        ...
        if "login.php" not in r.url:
            logger.info("SUCCESS: Session created and logged in.")
            return s
        else:
...
def fetch_form_tokens(session: requests.Session, target_id: str) -> Dict[str, str]:
    ...
    try:
        ...
        if "Security Check" in r.text or "Error" in r.text:
             logger.error("FATAL: Security Check/Error page received while fetching tokens. IP/Account blocked.")
             return {}
...
def send_message(session: requests.Session, target_id: str, message: str) -> bool:
    ...
    if not tokens:
        logger.warning("Tokens missing for target %s — cannot send. Session check needed.", target_id)
        return False

    # सबसे सरल POST URL का उपयोग करें
    send_url = f"https://mbasic.facebook.com/messages/send/?ids[{target_id}]=1"

    # --- Stealth Payload: केवल आवश्यक डेटा भेजें ---
    payload = {
        "body": message,
        **tokens,
        "send": "Send" # Facebook को Send बटन की आवश्यकता होती है
    }
    
    # अनावश्यक Payloads हटाएँ
    if 'tids' in payload: del payload['tids']
    if 'csid' in payload: del payload['csid']
    if 'ids' in payload: del payload['ids']
    if 'pass' in payload: del payload['pass'] # सुरक्षा के लिए
    ...
    try:
        ...
        if r.status_code == 302:
            ...
            if "Something Went Wrong" in r.text or "Error" in r.text or "Security Check" in r.text:
                logger.error("FINAL FAILURE: Facebook rejected POST request (IP/Security Block).")
                return False
            ...
            for t in targets:
                for m in messages:
                    ...
                    if ok:
                        ...
                        logger.warning("Failed to send. Check logs for FINAL FAILURE error.")
...
@app.route("/")
def index():
    return "Bot running. Mode: Stealth M-Basic (Final Attempt). Check logs."
